...
if apps_str:
    selected = [k for k in spec_info if spec_info[k]['base_name'] in apps_str.split()]
else:
...
for app in $SPEC_APPS; do
    ...
    "$QEMU" \
        ...
        || echo "  [WARN] $app checkpoint 退出码非0 (可能正常)"

    NUM_CPT=$(find "$ARCHIVE/checkpoint-0-0-0/$app" -name "*.gz" -type f 2>/dev/null | wc -l)
