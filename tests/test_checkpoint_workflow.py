...
class CheckpointWorkflowTests(unittest.TestCase):
    def test_workflow_dispatch_exposes_single_and_batch_inputs(self):
        ...
        self.assertIn("allow_new_archives", inputs)
        ...
        self.assertEqual(inputs["allow_new_archives"]["default"], "false")
        ...
        self.assertIn('ALLOW_NEW_ARCHIVES="${{ github.event.inputs.allow_new_archives }}"', run_script)
        ...
        self.assertIn('if [[ "$ALLOW_NEW_ARCHIVES" != "true" && "$ALLOW_NEW_ARCHIVES" != "false" ]]; then', run_script)
        self.assertIn('if [[ -n "$RESUME_AFTER" && "$RESUME_AFTER" != "auto" && "$RESUME_AFTER" != "profiling" && "$RESUME_AFTER" != "cluster" ]]; then', run_script)
        self.assertIn('if [[ -n "$BIN_LIST_PATH" && -n "$ARCHIVE_ID_INPUT" ]]; then', run_script)
        self.assertIn('if [[ -n "$RESUME_AFTER" && "$RESUME_AFTER" != "auto" && -z "$ARCHIVE_ID_INPUT" ]]; then', run_script)
        ...
        self.assertIn('--allow-new-archives', run_script)
        self.assertIn('--max-workers "$MAX_WORKERS"', run_script)
        self.assertIn('--bin-list "$BIN_LIST_PATH"', run_script)
        self.assertIn("- interval: \\`$INTERVAL\\`", run_script)
        self.assertIn("- resume_after: \\`${RESUME_AFTER:-fresh}\\`", run_script)
        self.assertIn("- allow_new_archives: \\`$ALLOW_NEW_ARCHIVES\\`", run_script)
